			...
			{  
				...
				{  
					...
					{ 
						...
						if (hit.transform.tag=="Laser"&&hit.transform.gameObject==player){
							hpPlayeraccess = hpmanager.GetComponent<HP> ();
							hpPlayeraccess.hpPlayer -= damage*Time.deltaTime;

						}
						if (hit.transform.tag=="Laser"&&hit.transform.gameObject==enemy){
							hpEnemyaccess = enemyhp.GetComponent<HPenemy> ();
							hpEnemyaccess.hp -= damage*Time.deltaTime;
						}
						if(nReflections==1)  
						{  
